...
const DetailsTarea = () => {
  ...
  const [idSubtarea, setIDSubtarea] = useState("");

  useEffect(() => {
    const detallesNames = async () => {
      try {
  ...
  const columns = [
    ...
    {
      name: "FECHA_INICIO",
      selector: (row) => obtenerFechasDB(row.FECHA_INICIO),
      sortable: true,
      width: "150px",
    },
    ...
    {
      name: "FECHA_FIN",
      selector: (row) => obtenerFechasDB(row.FECHA_FIN),
      sortable: true,
      width: "150px",
    },
