...
const CreateTarea = () => {
  ...
  useEffect(() => {
    const APIs = async () => {
      try {
        const responses = await Promise.all([
          ...
          fetch(
            `${API_Services}/TAREA/SelectTareaPredecesora/${selectedIdSubProyectos}`,
            { headers: { Authorization: `Bearer ${token}` } }
          ),
        ...
        setSubProyectos(subProyectoData.sort((a, b) => a.ID - b.ID));
        setArea(areaData.sort((a, b) => a.ID - b.ID));
        setTPredecesora(TareaPredecesora);
    ...
    APIs();
  }, [noCia, token, selectedIdProyectos, selectedIdSubProyectos]);
  ...
  // const validarSelectedIdTarea = (selectedIdTarea) => {
  //   if (Number.isInteger(selectedIdTarea) && selectedIdTarea > 0) {
  //     return selectedIdTarea;
  //   } else if (selectedIdTarea === "") {
  //     return 1;
  //   } else {
  //     return selectedIdTarea;
  //   }
  ...
  const saveTarea = async (nombre, descripcion, descripAdver) => {
    // debugger;
  ...
  const handleInputChange = (e) => {
    ...
    if (
      ...
      saveTarea(form.nombre, form.descripcion, form.descripAdver);
      toast.success("Tarea guardado exitosamente", {
        theme: "colored",
      });
                  ...
                  <div className="col-md-6 mb-4">
                    <div className="form-outline">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          setSelectedIdParte(e.target.value);
                        }}
                      >
