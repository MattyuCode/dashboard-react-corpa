...
const EditTarea = () => {
  ...
  //COMMENT:------------------------------------------>
  let [TPredec, setTPredec] = useState(0);
  const [selectedIdProyectos, setSelectedIdProyectos] = useState(null);
  const [selectedIdSubProyectos, setSelectedIdSubProyectos] = useState(null);
  const [selectedIdEtapa, setSelectedIdEtapa] = useState(null);
  const [selectedIdArea, setSelectedIdArea] = useState(null);
  const [selectedIdSubArea, setSelectedIdSubArea] = useState(null);
  const [selectedIdEquipo, setSelectedIdEquipo] = useState("");
  const [selectedIdParte, setSelectedIdParte] = useState(null);
  const [selectedIdActividad, setSelectedIdActividad] = useState(null);
  const [selectedIdResponsable, setSelectedIdResponsable] = useState(null);
  const [selectedOption, setSelectedOption] = useState(null);
  const [selectedIdAdvertencia, setSelectedIdAdvertencia] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const tareaPredecesoraDa = await fetch(
          `${API_Services}/TAREA/SelectTareaPredecesora/${tareaData[0].ID_SUBPROYECTO}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const dataTareaPrede = await tareaPredecesoraDa.json();
        setTPredecesora(dataTareaPrede);
        ...
        setTPredec(tareaData[0].ID_TAREA_PREDECESORA);
  ...
  const updateTarea = async (nombre, descripcion, descripAdver) => {
    // debugger;
    const requestOptions = {
      ...
      body: JSON.stringify({
        ...
        iD_TAREA_PREDECESORA: TPredec === "DEFAULT" ? (TPredec = "0") : TPredec,
  ...
  const handleInputChange = (e) => {
    ...
    if (
      ...
      updateTarea(form.nombre, form.descripcion, form.descripAdver);

      toast.success("Tarea Actualizado exitosamente", {
        theme: "colored",
      });
                  ...
                  <div className="col-md-6 mb-4">
                    <div className="form-outline">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          setSelectedIdEtapa(e.target.value);
                          // updateEtapa(e.target.value);
                        }}
                      >
                  ...
                  <div className="col-md-6 mb-4">
                    <div className="form-outline">
                      ...
                      <select
                        ...
                        // onChange={(e) => updateParte(e.target.value)}
                      >
