...
const CreateAdvertencia = () => {
  ...
  const guardarAdvertencia = async (name, idSub, descrip) => {
    ...
    try {
      const response = await fetch(
        `${API_Services}/LUGARADVERTENCIA/Insert`,
        requestOptions
      );
      const data = await response.json();
      console.log(data);
      Swal.fire({
        icon: "success",
        title: "Advertencia guardado",
        text: "Advertencia guardado exitosamente",
      }).then(() => {
        setRedirect(true);
      });
    } catch (error) {
  ...
  if (redirect) {
    return usenavigate("/lugarAdvertencia");
  }
...
export default CreateAdvertencia;
