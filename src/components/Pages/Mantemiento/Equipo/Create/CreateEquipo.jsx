...
const CreateEquipo = () => {
  ...
  async function fetchApiSubProyecto  (idproyecto)  {
    ...
    async function  fetchApiArea  (idproyecto)  {
      try {
        const response = await fetch(
          `${API_Services}/AREA/SelectIdSubProyecto/${noCia}/${idproyecto}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const data = await response.json();
        setArea(data);
    ...
    async  function  fetchApiSubArea  (idsubproyecto) {
      try {
        const response = await fetch(
          `${API_Services}/SUBAREA/SelectIdArea/${noCia}/${idsubproyecto}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const data = await response.json();
        setSubArea(data);
