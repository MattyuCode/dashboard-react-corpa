...
export const ResponsableContext = () => {
    ...
    useEffect(() => {
        const GET_EMPLEADOS = async () => {
            ...
            try {
                const url =
                    `${axios.getUri()}/api/RESPONSABLE/GetEmpleados/${noCia}`;
    ...
    const REFRESH_RESPONSABLE = async () => {
        debugger
        try {
            const url =
                `${axios.getUri()}/api/RESPONSABLE/SelectAllResponsable/${noCia}`;
  ...
  return { empleados, usuarios, CREATE_RESPONSABLE,responsables,deleteResponsable,REFRESH_RESPONSABLE};
};
